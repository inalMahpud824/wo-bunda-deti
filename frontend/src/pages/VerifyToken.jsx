...
const VerifyToken = () => {
  ...
  useEffect(() => {
    ...
    if(!login && localStorage.getItem("email") == null){
  ...
  const handleSubmit = async (e) => {
    ...
    e.preventDefault();
    try {
      await instance.post("api/auth/forgot-password/token", {
        email: localStorage.getItem("email"),
        token: input.token,
      });
      localStorage.setItem("otp", input.token);
    ...
    <>
      ...
      <section className="w-full min-h-screen bg-white flex flex-col items-center justify-center relative">
        ...
        >
          ...
          <label htmlFor="token" className="py-2 font-semibold">
            Token
          </label>
          <input
            type="text"
            placeholder="Masukan token yang dikirim ke email"
            name="token"
